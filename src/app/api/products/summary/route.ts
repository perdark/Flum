...
import { products, inventoryTemplates, inventoryItems, productPlatformLinks, platforms } from "@/db/schema";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (platformId) {
      ...
      const subQuery = db
        .select({ productId: productPlatformLinks.productId })
        .from(productPlatformLinks)
        .where(eq(productPlatformLinks.platformId, platformId));
